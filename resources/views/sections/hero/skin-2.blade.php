			...
			<div class="col-sm-10 col-lg-8 col-xl-6 all-text-white my-5 mt-md-0">
				<div class="py-0 py-md-5 my-5">
					...
					<div class="d-inline-block bg-white px-3 py-2 rounded-pill mb-3">
						<p class="mb-0 text-dark"><span class="badge text-bg-success rounded-pill me-1">{{ tr('New') }}</span>	{{ tr('One to one video call using web browser') }}</p>
					</div>

					<!-- Title -->
					<h1 class="text-white display-5">{{ tr('Start learning from') }} <span class="text-warning">{{ tr('best institutions') }}</span></h1>
					<p class="text-white">{{ tr('Demesne far-hearted suppose venture excited see had has. Dependent on so extremely delivered by. Yet no jokes worse her why.') }}</p>

					<div class="d-sm-flex align-items-center mt-4">
						<!-- Button -->
						<a href="#" class="btn btn-primary me-2 mb-4 mb-sm-0">{{ tr('Get Started') }}</a>
						<!-- Video button -->
