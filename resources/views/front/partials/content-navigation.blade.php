...
<div class="piksera-course-player-curriculum">
    <div class="piksera-course-player-curriculum__wrapper">
        <div class="piksera-course-player-curriculum__mobile-header">
            <h3 class="piksera-course-player-curriculum__mobile-title"> Curriculum</h3> <span
                class="piksera-course-player-curriculum__mobile-close"></span>
        </div>
                ...
                @if ($auth_user)
                    ...
                    <div class="lecture-page-course-progress mb-4 px-4 p-20 text-center">
                        <div class="progress mb-3">
                            <div class="progress-bar bg-primary" style="width: {{ $completed_percent }}%"></div>
                        </div>
                    ...
                    @if (is_array($active_plugins) && in_array('Certificate', $active_plugins))
                        ...
                        @if (Auth::check())
                            ...
                            <div class="mb-4 text-center">
                                <a href="{{ $certURL }}" target="_blank" class="btn btn-primary">
                                    <i class="la la-certificate"></i> {{ tr('Download Certificate') }}
                                </a>
                            </div>
                        @endif
                            ...
                            <span class="section-name flex-grow-1 ml-2 d-flex">
                                ...
                                @if (
                                    $auth_user &&
                                        $drip_items &&
                                        is_array($drip_items) &&
                                        isset($drip_items['sections']) &&
                                        in_array($section->id, $drip_items['sections']))
                        ...
                        <div class="course-section-body">
                            ...
                            @if ($section->items->count())
                                @foreach ($section->items as $item)
                                    ...
                                    <div
                                        class="sidebar-section-item {{ $item->id == $content->id ? 'active' : '' }} {{ $is_completed ? 'completed' : '' }}">
                                        <div class="section-item-title ">

                                            <a href="{{ route('single_' . $item->item_type, [$course->slug, $item->id]) }}"
                                                class="piksera-curriculum-accordion__section"
                                                @if ($is_completed) data-toggle="tooltip" title="{{ __t('completed') }}" @endif>
                                                ...
                                                <div class="title-container pl-2 flex-grow-1 d-flex">
                                                    ...
                                                    <span
                                                        class="lecture-name flex-grow-1 piksera-curriculum-accordion__title-wrapper">
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('save_review', $course->id) }}" method="post">
                    ...
                    <div class="review-modal-footer p-3">
                        <p class="review-modal-nofity-text">
                            <i class="la la-globe"></i> {{ tr('Your review will be posted publicly. Under') }}
                            <strong>{{ $auth_user->name }} {{ $auth_user->last_name }}</strong>
                        </p>
                        ...
                        <button type="button" class="btn btn-secondary"
                            data-bs-dismiss="modal">{{ __t('cancel') }}</button>
                    </div>
                </form>
            </div>
        </div>
