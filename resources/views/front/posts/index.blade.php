                    ...
                    <div class="d-flex justify-content-center position-relative">
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb mb-0">
                                ...
                                <li class="breadcrumb-item active" aria-current="page">{{ tr('Library') }}</li>
                            </ol>
                        </nav>
                    </div>
                </div>
...
<div class="col-sm-6 col-lg-4 col-xl-3">
    <div class="card bg-transparent">
        <div class="overflow-hidden rounded-3">
            <!-- Use the post's featured_image if available -->
            <img src="{{ asset($post->featured_image ?: '/assets/images/placeholder-image.png') }}" class="card-img" alt="course image">
            <!-- Overlay -->
            <div class="bg-overlay bg-dark opacity-4"></div>
            <div class="card-img-overlay d-flex align-items-start p-3">
                <!-- badge -->
                <a href="#" class="badge text-bg-danger">{{ $post->topic }}</a>
            </div>
        </div>
        ...
        <div class="card-body">
            <!-- Title -->
            <!-- Use route() to generate the URL for the post -->
            <h5 class="card-title"><a href="{{ route('posts.show', $post->slug) }}">{{ $post->title }}</a></h5>
            <!-- Content -->
            <p class="text-truncate-2">{{ Str::limit($post->short_description, 95) }}</p>
            <!-- Info -->
            <div class="d-flex justify-content-between">
                <h6 class="mb-0"><a href="#">{{ $post->user->name }}</a></h6>
                <!-- Assuming created_at field exists on your post model -->
