    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">{{ tr('Question Type') }} - {{question_types($question->type)}}</h5>
                                            ...
                                            <div class="question-opt p-2 my-2" data-type="radio">
                                                ...
                                                <div class="form-group m-0">
                                                    <div class="input-group mb-3">
                                                        ...
                                                        <input type="text" class="form-control" name="options[{{$loop->iteration}}][title]" value="{{$option->title}}" placeholder="{{ tr('Option title') }}">
                                                    ...
                                                    <div class="question-opt-footer d-flex mt-3">
                                                        ...
                                                        <div class="question-opt-display flex-grow-1 ml-4">
                                                            <p class="mb-2">{{ tr('Display preference') }}</p>

                                                            <label class="mr-2">
                                                                <input type="radio" name="options[{{$loop->iteration}}][d_pref]" value="text" {{checked('text', $option->d_pref)}}> {{ tr('Text') }}
                                                            </label>
                                                            {{-- <label class="mr-2">
                                                                <input type="radio" name="options[{{$loop->iteration}}][d_pref]" value="image" {{checked('image', $option->d_pref)}} > {{ tr('Image') }}
                                                            </label>
                                                            <label class="mr-2">
                                                                <input type="radio" name="options[{{$loop->iteration}}][d_pref]" value="both" {{checked('both', $option->d_pref)}} > {{ tr('Both') }}
                                                            </label> --}}
                                                        </div>

                                                        <label class="m-0 mt-1 text-right checkbox">
                                                            {{ tr('Correct answer') }} <input type="checkbox" class="is_correct_input" name="options[{{$loop->iteration}}][is_correct]" value="1" {{checked('1', $option->is_correct)}} >
                                                            <span></span>
                                                        </label>
                                                    </div>
                                                </div>
