    ...
    <form class="curriculum-edit-quiz-form" action="{{route('update_quiz', [$item->course_id, $item->id])}}" method="post">
        ...
        <div id="quiz-basic" class="section-item-tab-wrap" style="display: block;">
            ...
            <div class="form-group mt-3">
                ...
                <button type="submit" class=" btn btn-primary btn-edit-quiz"  name="save" value="save_next"> <i class="la la-save"></i> {{__t('save_'.$item->item_type)}}</button>
            </div>
        ...
        <div id="quiz-questions" class="section-item-tab-wrap mb-5" style="display: none;">
            ...
            <div class="d-flex align-items-center mt-2 mt-md-0">
                <a href="#" class="btn btn-sm btn-primary mb-0" data-bs-toggle="modal" data-bs-target="#addQuiz">{{ tr('Add New Question') }}</a>
            </div>
        ...
        <div id="quiz-settings" class="section-item-tab-wrap" style="display: none;">
            <div id="quiz-settings-wrap">
                <div class="row mb-3">
                    <label class="col-sm-4 col-form-label">{{ tr('Gradable') }}</label>
                    <div class="col-sm-8">
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" id="quiz-gradable" name="quiz_gradable" {{ $item->quiz_gradable ? 'checked' : '' }}>
                            <label class="form-check-label" for="quiz-gradable">{{ tr('Quiz Gradable') }}</label>
                        </div>
                        <p class="text-muted"><small>{{ tr('If this quiz test affects the student\'s grading system for this course.') }}</small></p>
                    </div>
                </div>


                <div class="row mb-3">
                    <label class="col-sm-4 col-form-label">{{ tr('Remaining time display') }}</label>
                    <div class="col-sm-8">
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" id="show-time" name="quiz_option[show_time]" {{ $item->option('show_time') ? 'checked' : '' }}>
                            <label class="form-check-label" for="show-time">{{ tr('Show Time') }}</label>
                        </div>
                        <p class="text-muted"><small>{{ tr('By enabling this option, the quiz taker will see the remaining time during the attempt.') }}</small></p>
                    </div>
                </div>
                ...
                <div class="row mb-3">
                    <div class="col-md-4">
                        <label class="form-label">{{ tr('Time Limit') }}</label>
                        <div class="input-group">
                            <input type="number" class="form-control" name="quiz_option[time_limit]" value="{{$item->option('time_limit')}}">
                            <span class="input-group-text">{{ tr('Minutes') }}</span>
                        </div>
                        <p class="text-muted"><small>{{ tr('Set zero to disable time limit.') }}</small></p>
                    </div>

                    <div class="col-md-4">
                        <label class="form-label">{{ tr('Passing Score (%)') }}</label>
                        <input type="number" class="form-control" name="quiz_option[passing_score]" value="{{$item->option('passing_score')}}">
                        <p class="text-muted"><small>{{ tr('Students have to achieve this score in percentage to pass this quiz.') }}</small></p>
                    </div>
                ...
                <div class="row">
                    <div class="col-md-12">
                        <button type="button" class="btn btn-outline-info btn-cancel-form me-2">{{__t('cancel')}}</button>
                        <button type="submit" class="btn btn-primary btn-edit-quiz"><i class="la la-save me-2"></i>{{__t('save_'.$item->item_type)}}</button>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="question-opt p-2 my-2" data-type="radio">
                    <div class="form-group m-0">
                        ...
                        <div class="question-opt-footer d-flex mt-3">
                            {{-- <div class="question-opt-image w-50">
...
<!-- Question Form Modal -->
            ...
            <div class="modal" id="addQuiz" tabindex="-1" aria-labelledby="addQuizLabel" aria-hidden="true">
                <div class="modal-dialog modal-xl">
                    <div class="modal-content">
                        <div class="modal-header bg-dark">
                            <h5 class="modal-title text-white" id="addQuizLabel">Question Type </h5>
                ...
                <div class="modal-footer">
                    <button type="submit" class="btn btn-purple"><i class="la la-save"></i> Save Question</button>
                     <button type="button" class="btn btn-secondary my-0" data-bs-dismiss="modal">{{ tr('Close') }}</button>
                </div>
            </form>
