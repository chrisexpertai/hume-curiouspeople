<div class="section-item-form-html p-4 border">
    ...
    <form class="update-assignment-form" action="{{route('update_assignment', [$item->course_id, $item->id])}}" method="post">
        ...
        <div class="form-group">
            <label for="notice">{{__t('notice')}}</label>
            <textarea name="notice" class="form-control" rows="5">{!! $item->notice !!}</textarea>
        </div>
                    ...
                    <div class="form-row">
                        ...
                        <div class="col">
                            <select class="form-control" name="assignment_option[time_duration][time_type]">
                                <option value="weeks" {{selected($item->option('time_duration.time_type'), 'weeks')}} >{{ tr('Weeks') }}</option>
                                <option value="days" {{selected($item->option('time_duration.time_type'), 'days')}}>{{ tr('Weeks') }}</option>
                                <option value="hours" {{selected($item->option('time_duration.time_type'), 'hours')}}>{{ tr('Weeks') }}</option>
                            </select>
                        </div>
                    </div>
        ...
        <div class="form-group text-right">
            ...
            <button type="submit" class=" btn btn-primary btn-save-assignment"  name="save" value="save_next"> <i class="la la-save"></i> {{__t('save_assignment')}}</button>
        </div>
    </form>
</div>
