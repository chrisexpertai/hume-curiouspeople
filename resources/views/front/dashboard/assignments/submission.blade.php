...
@section('content')
    ...
    <div class="assignment-submission-wrap">
        ...
        <div class="assignment-submission-header mb-4 alert alert-light">
            ...
            <a href="{{course_url($submission->course)}}">{{$submission->course->title}}</a>
        @else
            {{__t('Course not available')}}
        @endif            <h4>{{ tr('Assignment') }}: <a href="{{route('single_assignment', [$submission->course->slug ?? '', $submission->assignment_id ?? ''])}}" >{{$submission->assignment->title ?? ''}}</a> </h4>
            ...
            @if($submission->is_evaluated)
                <div class="alert alert-success mt-4">
                    ...
                    <p class="text-muted"> {{ tr('Evaluated at') }}: {{$submission->evaluated_at->format(date_time_format())}}</p>
                </div>
            @endif
        ...
        <div class="assignment-submission-text-wrap border rounded-3 p-3 my-3">
        ...
        @if($submission->attachments->count())
            <div class="lecture-attachments border rounded-3 p-3 mt-4">
        ...
        <div class="submission-evaluation-form p-4 border rounded my-4">

            <form action="" method="post">
                @csrf

                <h4 class="mb-4"><i class="la la-book-reader"></i> {{ tr('Assignment Evaluation Form') }}</h4>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-9">
                        ...
                        <small class="form-text text-muted">{{ tr('Write a note about what are you thinking about this submission and you would like to let this student know. such as "You need to improved your skills') }}"</small>
                    </div>
                </div>

                <div class="form-group row">
                    <div class="col-sm-9 offset-sm-3">
                        <button type="submit" class="btn mt-3 btn-primary btn-lg">{{ tr('Evaluate this submission') }}</button>
                    </div>
                </div>
