...
function initializeCKEditor(element) {
    ...
    ClassicEditor
        .create(element, {
            plugins: [
                Essentials, Bold, Font, BlockQuote, CKFinder, CKFinderUploadAdapter,
                CloudServices, Heading, Image, ImageCaption, ImageResize, ImageStyle,
                ImageToolbar, ImageUpload, Base64UploadAdapter, Indent, IndentBlock, Italic,
                Link, List, MediaEmbed, Mention, Paragraph, PasteFromOffice, PictureEditing,
...
$(function () {
    ...
    $(document).on('click', '.add-item-lecture', function (e) {
        ...
        initializeCKEditor(document.querySelector('textarea.ajaxCkeditor'));

        // ClassicEditor
        //     .create(document.querySelector('textarea.ajaxCkeditor'))
        //     .then(editor => {
        //         console.log(editor);
        //     })
        //     .catch(error => {
    ...
    $(document).on('click', '.create-new-quiz', function (e) {
        ...
        $that.closest('.dashboard-course-section').find('.section-item-form-wrap').html($formHtml);
        initializeCKEditor(document.querySelector('.ajaxCkeditor'));

        // ClassicEditor
        //     .create(document.querySelector('.ajaxCkeditor'))
        //     .then(editor => {
        //         console.log(editor);
        //     })
        //     .catch(error => {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success) {
                    ...
                    $form.find('#questionRequestResponse').empty();
                    // Close the modal
                    $('#addQuiz').modal('hide');
                    // Load quiz questions if needed
    ...
    $(document).on('click', '.browse-enter-nav-link', function (e) {
        e.preventDefault();
        var $ul = $('.enter-ul-first');
        if ($ul.hasClass('d-block')) {
            $ul.removeClass('d-block').addClass('d-none');
        } else {
            $ul.removeClass('d-none').addClass('d-block');
    ...
    if ($('#videoFormModalShouldOpen').length) {
        $('#videoFormModalShouldOpen').modal('show');
    }
 ...
 */
    $(document).on('mouseenter', '.review-write-star-wrap i', function () {
        var $parentWrap = $(this).closest('.review-write-star-wrap');
        var ratingValue = $(this).attr('data-rating-value');

        // Reset all stars to empty
        $parentWrap.find('i').removeClass('fas fa-star').addClass('far fa-star');

        // Fill stars up to the hovered one
        for (var i = 1; i <= ratingValue; i++) {
            $parentWrap.find('i[data-rating-value="' + i + '"]').removeClass('far fa-star').addClass('fas fa-star');
        }

        // Set the value in the input field
        $parentWrap.find('input[name="rating_value"]').val(ratingValue);
    });
    ...
    $(document).on('click', '.video-demo', function () {
        if (!pageData.is_logged_in) {
            $('#videoFormModal').modal('show');
            return false;
        }
