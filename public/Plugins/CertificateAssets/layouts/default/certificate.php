...
<body>
    ...
    <div id="watermark">
        <?php
        $signature_template = get_option('certificate.template');
        $signature_template_src = null;
        if ($signature_template) {
            $signature_template_src = media_image_uri($signature_template)->original;
        }
        
        if ($signature_template_src) { 
            
          echo "<img src='{$signature_template_src}' width='100%' height='100%'/>";
