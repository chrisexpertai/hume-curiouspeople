...
class Media extends Model
{
    ...
    public function getMediaInfoAttribute()
    {
        ...
        $info = [
            ...
            'url' => $url_path,
            'thumbnail' => $url_path,
            'uploaded_at' => $this->created_at ? $this->created_at->format(get_option('date_format')) : \date("Y-m-d H:i:s"),
        ];
    ...
    {
        $thumbnail_url = asset('assets/images/placeholder-image.png');
        if (substr($this->mime_type, 0, 5) == 'image') {
            $thumbnail_url = media_image_uri($this)->thumbnail;
        }
        if (substr($this->mime_type, 0, 5) == 'audio') {
