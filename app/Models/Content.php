...
class Content extends Model
{
    ...
    public function getDripAttribute(){
       ...
       // Check if the user has an active subscription for the course, excluding course owners
        if ($this->course->subscription_plan_id && Auth::check()) {
            $user = Auth::user();

            // Check if the user is the course owner
            if ($user->id !== $this->course->user_id) {
                $userPlanId = $user->subscription_plan;

                // Check if the user is subscribed to the course's specific subscription plan
                $isSubscribedToCoursePlan = $user->isSubscribedToPlan($this->course->subscription_plan_id);

                if (!$isSubscribedToCoursePlan) {
                    $data['is_lock'] = true;
                    $data['message'] = 'This content is only available with a specific subscription plan.';
                }
            }
        }
