...
class Course extends Model
{
    ...
    public function getThumbnailUrlAttribute(){
        return media_image_uri($this->media)->thumbnail;
    }
    ...
    public function getSubscribtionAttribute($value){
        return $this->price_plan === 'subscription';
    }
    ...
    public function enrolls()
{
    ...
    public function getContinueUrlAttribute(){
      ...
      // Define a relationship with SubscriptionPlan
    ...
    public function price_html($originalPriceOnRight = false, $showOff = false){
        ...
        $price_html = "<div class=' {$priceLocation}'>";

        if ($this->subscribtion && $this->subscription_plan) {
            $price_html .= "<span class='subscription-plan'>Plan: " . $this->subscription_plan->name . "</span>";
        } else if ($this->paid && $this->price > 0){
    ...
    public function status_html($badge = true){
        ...
        $class = $badge ? 'badge bg-warning badge' : 'status-text text';
        ...
        switch ($status){
            case 1:
                ...
                break;
            case 2:
                $html = "<span class='bg-blue p-1 rounded-3 status-text {$class}-info'> <i class='la la-clock-o'></i> ".__t('pending')."</span>";
                break;
            case 3:
        ...
        if ($this->is_popular){
            $html .= "<span class='badge bg-secondary badge-primary mx-2' data-toggle='tooltip' title='Popular'> <i class='la la-bolt'></i></span>";
        }
        if ($this->is_featured){
            $html .= "<span class='badge bg-primary badge-info mx-2'  data-toggle='tooltip' title='Featured'> <i class='la la-bookmark'></i></span>";
        }
    ...
    public function getDripItemsAttribute(){
        ...
        foreach ($dripSections as $dripSection){
            ...
            if ($dripSection->unlock_days && $dripSection->unlock_days > 0 ){
...
public function paid_price($originalPriceOnRight = false, $showOff = false){
    $current_price = '';
