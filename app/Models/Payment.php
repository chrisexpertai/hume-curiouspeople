...
class Payment extends Model
{
    ...
    {
        return $this->belongsToMany(SubscriptionPlan::class, 'subscription_payments');
    }

    public function do_subscribe($cart_subscription)
{
    $carbon = Carbon::now()->toDateTimeString();
    $data = [
        'subscription_plan_id' => $cart_subscription['plan_id'],
        'user_id'              => $this->user_id,
        'subscription_price'   => $cart_subscription['price'],
        'payment_id'           => $this->id,
        'status'               => $this->status,
        'subscribed_at'        => $carbon
    ];
    DB::table('subscription_payments')->insert($data);
    ...
    public static function create_and_sync($data)
{
    ...
    // If any fees, add them to Payment
    ...
    if (is_array($cart->subscriptions) && count($cart->subscriptions)) {
        foreach ($cart->subscriptions as $subscription) {
            $payment->do_subscribe($subscription)->distribute_earning();
        }
    }
...
public function distribute_earning()
{
    ...
    if ($enable_instructors_earning) {
        ...
        if ($enrolls->count()) {
            foreach ($enrolls as $enroll) {
                // Distribute earnings for course enrollments
        ...
        $subscriptions = DB::table('subscription_payments')->wherePaymentId($this->id)->get();
    ...
    public function getStatusContextAttribute(){
        ...
        $html = "<span class='badge text-{$statusClass} payment-status-{$this->status} badge-{$statusClass}'> <i class='la la-{$iclass}'></i> {$this->status}</span>";
        return $html;
    }
