...
class DashboardController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $enrolledCoursesCollection = $auth_user->enrolls()->with('category')->get()->sortByDesc('created_at');
        $enrolledIds = $enrolledCoursesCollection->pluck('id');
        $courseCollection = $enrolledCoursesCollection;
        $emptyMessage = __("You haven't enrolled in a course yet.");

        $completedCount = $enrolledCoursesCollection->filter(function ($course) use ($auth_user) {
            return $course->completed_percent($auth_user) >= 100;
        })->count();

        $enrolledIncompleteCount = $enrolledCoursesCollection->filter(function ($course) use ($auth_user) {
            return $course->completed_percent($auth_user) < 100;
        })->count();

        $availableCount = Course::publish()->whereNotIn('id', $enrolledIds)->count();

        $filterCounts = [
            'my' => $enrolledCoursesCollection->count(),
            'completed' => $completedCount,
            'enrolled' => $enrolledIncompleteCount,
            'available' => $availableCount,
        ];

        if ($filter === 'completed') {
            $courseCollection = $enrolledCoursesCollection->filter(function ($course) use ($auth_user) {
                return $course->completed_percent($auth_user) >= 100;
            });
            $emptyMessage = __("You haven't completed any courses yet.");
        } elseif ($filter === 'enrolled') {
            $courseCollection = $enrolledCoursesCollection->filter(function ($course) use ($auth_user) {
                return $course->completed_percent($auth_user) < 100;
            });
            $emptyMessage = __("You haven't started any courses yet.");
        } elseif ($filter === 'available') {
            $enrolledIds = $enrolledCoursesCollection->pluck('id');
            $enrolledCourses = Course::publish()
                ->whereNotIn('id', $enrolledIds)
                ->orderBy('created_at', 'desc')
                ->paginate(8);
            $emptyMessage = __('No additional courses available right now.');
        }

        if (!isset($enrolledCourses)) {
            $enrolledCourses = $this->paginateCollection($courseCollection, 8);
        }
    ...
    protected function paginateCollection(Collection $items, int $perPage = 8): LengthAwarePaginator
    {
