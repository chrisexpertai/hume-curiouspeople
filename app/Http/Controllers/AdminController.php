...
class AdminController extends Controller
{
    ...
    public function index()
    {
        ...
        $tickets = Ticket::latest()->take(4)->get();
        $instructors = User::where('user_type', 'instructor')->get();
        $websiteViews = WebsiteView::count(); // Assuming you have a model for website views
        $websiteViewsCount = WebsiteView::count();

        // Get the currently authenticated admin user
        $adminUser = auth()->user();

        // Get the count of unread notifications for the admin user
        $unreadNotificationsCount = $adminUser->notifications()->where('read', 0)->count();

        // Get all unread notifications for the admin user
        $unreadAdminNotifications = $adminUser->notifications()->where('read', 0)->get();
        ...
        return view('admin.dashboard', compact('title', 'unreadNotificationsCount', 'unreadAdminNotifications', 'websiteViewsCount', 'pendingApplicationsCount', 'chartData', 'tickets', 'websiteViews', 'instructors'));
    }
    ...
    public function adminCourses(Request $request)
    {
        ...
        $title = __a('courses');
        $courses = Course::query(); // Removed the where('status', '>', 0) condition to include all statuses
