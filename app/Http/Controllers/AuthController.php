...
class AuthController extends Controller
{
    ...
    public function login(){
        $title = __t('login');
        return view_template('auth.login', compact('title'));
    }
    ...
    public function register(){
        $title = __t('signup');
        return view_template('auth.register', compact('title'));
    }
    ...
    public function passwordResetForm(){
        ...
        return view(('auth.reset_form'), compact('title'));
    }
    ...
    public function getSocialUser($providerUser, $provider = '') {
        ...
        if ($user) {
            ...
            if ($user) {
                // Update provider details if the user already exists
                $user->provider_user_id = $providerUser->getId();
                $user->provider = $provider;
                $user->save();
            } else {
                // Create a new user with email as password
                $user = User::create([
                    'email'             => $providerUser->getEmail(),
                    'password'          => bcrypt($providerUser->getEmail()), // Set password as email
